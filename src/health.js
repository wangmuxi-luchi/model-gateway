...
export class HealthStore {
  ...
  claimHalfOpen(candidate, model) {
    const key = this.key(candidate, model); const entry = this.entries.get(key)
    if (!entry || entry.status !== "cooldown" || !entry.nextProbeAt || entry.nextProbeAt > Date.now() || (entry.inFlight ?? 0) > 0) return false
    this.entries.set(key, { ...entry, inFlight: 1 })
    return true
  }
  release(candidate, model) {
    const key = this.key(candidate, model); const entry = this.entries.get(key)
    if (entry) this.entries.set(key, { ...entry, inFlight: 0 })
  }
  latest(candidate) {
